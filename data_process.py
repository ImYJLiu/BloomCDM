...
def create_user_list(df, user_size, exer_size):
    user_list = [list() for u in range(user_size)] #学生做过的题目。[[做过题目1 ，题目2 ，题目3], [题目1 ，题目2]]
    user_right_list = [list() for u in range(user_size)] # 学生做对的题目。 [[做对题目1 ，题目2 ，题目3], [题目1 ，题目2]]
    user_dict_list = [list() for u in range(user_size)] # 学生题目详细得分  [[{做过题目1：0}， {做过题目2：1}],[{做过题目1：1}]]
    user_item_score = np.zeros((user_size, exer_size)) # 学生在题目上的得分
...
def split_train_test(df, user_size, exer_size, test_size):
...
def main(args):
    ...
    exer_size = len(df['exer'].unique())
    know_size = len(df['sole_know'].unique())


    print('Complete compute the size of user, exer')

    train_user_list, train_right_list, train_user_dict_list, train_user_item_score, train_df, test_user_list, test_right_list, test_user_dict_list, test_user_item_score, test_df = split_train_test(
        df, user_size, exer_size, args.split_rate)
    print('Complete spliting items for training and testing and generate list')

    know_group = create_group_1(train_df)  # 通过json的形式，存放知识点：知识组平均分
    all_sole_dict = dict(train_df['sole_know'].groupby(train_df['user']).unique()) # 存放着二级每个人对应的知识点（R矩阵的大小）(算是简化版的know_group) 每个人：考察的知识点
    all_exer_sole = dict(train_df['exer'].groupby(train_df['sole_know']).unique()) # 存放
    higher_know_group, higher_know_size, all_higher_sole_dict = create_group_2(user_size, know_size,
                                                                               know_group)  # 通过json的形式，存放组信息和平均分（R矩阵的每一个值）
    print('Complete generete know group and sole_dict/list for each level')

    train_dict_pair = create_pair(train_user_dict_list)
    train_pair = create_pair(train_right_list)
    print('Complete creating pair')
    print('======================================================')
    dataset = {'user_size': user_size, 'exer_size': exer_size, 'know_size': know_size,'higher_know_size': higher_know_size,
               'user_mapping': user_mapping, 'exer_mapping': exer_mapping, 'know_mapping': know_mapping,
               'train_user_list': train_user_list, 'test_user_list': test_user_list,
               'train_right_list': train_right_list, 'test_right_list': test_right_list,
               'train_pair': train_pair, 'know_group': know_group, 'higher_know_group': higher_know_group,
               'all_sole_dict': all_sole_dict, 'all_higher_sole_dict': all_higher_sole_dict,
               'train_user_item_score':train_user_item_score, 'test_user_item_score':test_user_item_score,
               'train_dict_pair':train_dict_pair}
...
if __name__ == '__main__':
    ...
    parser.add_argument("-tdd", "--timedivided", type=int,help="The number of timedivided.")
    parser.add_argument("-sr", "--split_rate", default=0.3,
                        type=int,help="Split_rate.")
